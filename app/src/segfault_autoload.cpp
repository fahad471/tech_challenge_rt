...
namespace
{
    ...
    {
        const auto path =
            executable_directory() / "plugin_segfault.dll";
        ...
        const auto init = reinterpret_cast<plugin_init_fn>(
            GetProcAddress(handle, "plugin_init"));
    ...
    {
        const auto path =
            executable_directory() / ".." / "lib" / "libplugin_segfault.so";
        ...
        const auto init =
            reinterpret_cast<plugin_init_fn>(dlsym(handle, "plugin_init"));
    ...
    {
        SegfaultPluginAutoload()
        {
            load_segfault_plugin();
        }
    };
